...
RegisterNetEvent('qb-vehicleshop:server:customTestDrive', function(vehicle, playerid)
    ...
    if #(GetEntityCoords(GetPlayerPed(src))-GetEntityCoords(GetPlayerPed(target)))<3 then
        ...
        TriggerClientEvent('QBCore:Notify', src, Lang:t('error.not_close'), 'error')
    end
end)
...
RegisterNetEvent('qb-vehicleshop:server:financePayment', function(paymentAmount, vehData)
    ...
    if newBalance > 0 then
        if player and paymentAmount >= minPayment then
            ...
            TriggerClientEvent('QBCore:Notify', src, Lang:t('error.min_allowed', {min = comma_value(minPayment)}), 'error')
        end
    else
...
RegisterNetEvent('qb-vehicleshop:server:financePaymentFull', function(data)
    ...
    if player and vehBalance ~= 0 then
        ...
        TriggerClientEvent('QBCore:Notify', src, Lang:t('error.already_payoff'), 'error')
    end
end)
...
RegisterNetEvent('qb-vehicleshop:server:buyShowroomVehicle', function(vehicle)
    ...
    if cash > vehiclePrice then
        ...
        TriggerClientEvent('QBCore:Notify', src, Lang:t('success.purchase'), 'success')
...
RegisterNetEvent('qb-vehicleshop:server:financeVehicle', function(downPayment, paymentAmount, vehicle)
    ...
    if downPayment > vehiclePrice then return TriggerClientEvent('QBCore:Notify', src, Lang:t('error.not_that_much'), 'error') end
    if downPayment < minDown then return TriggerClientEvent('QBCore:Notify', src, Lang:t('error.downPayment_small'), 'error') end
    if paymentAmount > Config.MaximumPayments then return TriggerClientEvent('QBCore:Notify', src, Lang:t('error.exceeded_max_pay'), 'error') end
    local plate = GeneratePlate()
...
RegisterNetEvent('qb-vehicleshop:server:sellShowroomVehicle', function(data, playerid)
    ...
    if #(GetEntityCoords(GetPlayerPed(src))-GetEntityCoords(GetPlayerPed(target.PlayerData.source)))<3 then
        ...
        if cash >= vehiclePrice then
            ...
            TriggerClientEvent('QBCore:Notify', src, Lang:t('success.commission', {commission = comma_value(commission)}), 'success')
            exports['qb-management']:AddMoney(player.PlayerData.job.name, vehiclePrice)
            TriggerClientEvent('QBCore:Notify', target.PlayerData.source, Lang:t('success.purchase'), 'success')
        elseif bank >= vehiclePrice then
            MySQL.Async.insert('INSERT INTO player_vehicles (license, citizenid, vehicle, hash, mods, plate, state) VALUES (?, ?, ?, ?, ?, ?, ?)', {
                target.PlayerData.license,
                cid,
...
RegisterNetEvent('qb-vehicleshop:server:checkFinance', function()
    ...
    if paymentDue then
        TriggerClientEvent('QBCore:Notify', src, Lang:t('info.payment_warning', {time = Config.PaymentWarning}))

...
RegisterNetEvent('qb-vehicleshop:client:TestDrive', function()
    if not inTestDrive and ClosestVehicle ~= 0 then
        ...
        QBCore.Functions.SpawnVehicle(Config.Shops[getShopInsideOf()]["ShowroomVehicles"][ClosestVehicle].chosenVehicle, function(veh)
            ...
            testDriveVeh = veh
            QBCore.Functions.Notify(Lang:t('info.time_limit', {time = Config.Shops[closestShop]["TestDriveTimeLimit"]}))
            SetTimeout(Config.Shops[closestShop]["TestDriveTimeLimit"] * 60000, function()
                if testDriveVeh ~= 0 then
                    ...
                    QBCore.Functions.Notify(Lang:t('info.test_complete'))
                end
            end)
        ...
        QBCore.Functions.Notify(Lang:t('error.test_already'), 'error')
    end
end)
...
RegisterNetEvent('qb-vehicleshop:client:customTestDrive', function(data)
    if not inTestDrive then
        ...
        QBCore.Functions.SpawnVehicle(vehicle, function(veh)
            ...
            TriggerServerEvent('qb-vehicletuning:server:SaveVehicleProps', QBCore.Functions.GetVehicleProperties(veh))
            testDriveVeh = veh
            QBCore.Functions.Notify(Lang:t('info.time_limit', {time = Config.Shops[shopInsideOf]["TestDriveTimeLimit"]}))
...
RegisterNetEvent('qb-vehicleshop:client:TestDriveReturn', function()
    ...
    if veh == testDriveVeh then
        ...
        QBCore.Functions.Notify(Lang:t('error.not_test_veh'), 'error')
    end
end)
    ...
    for k,v in pairs(QBCore.Shared.Vehicles) do
        if QBCore.Shared.Vehicles[k]["category"] == data.catName and QBCore.Shared.Vehicles[k]["shop"] == getShopInsideOf() then
            vehicleMenu[#vehicleMenu + 1] = {
                header = v.name,
                txt = Lang:t('menu.price', {price = v.price}),
                params = {
                    isServer = true,
        ...
        inputs = {
            {
                ...
                isRequired = true,
                name = 'downPayment',
                text = Lang:t('menu.downPayment', {min = Config.MinimumDown})
            },
            {
                type = 'number',
                isRequired = true,
                name = 'paymentAmount',
                text = Lang:t('menu.paymentAmount', {max = Config.MaximumPayments})
            }
        }
    })
    if dialog then
        ...
        inputs = {
            {
                ...
                isRequired = true,
                name = 'downPayment',
                text = Lang:t('menu.downPayment', {min = 10})
            },
            {
                type = 'number',
                isRequired = true,
...
RegisterNetEvent('qb-vehicleshop:client:getVehicleFinance', function(data)
    local vehFinance = {
        ...
        {
            isMenuHeader = true,
            header = Lang:t('menu.balance'),
            txt = '$'..comma_value(data.balance)
        },
        {
            isMenuHeader = true,
            header = Lang:t('menu.payments'),
            txt = data.paymentsLeft
        },
        {
            isMenuHeader = true,
            header = Lang:t('menu.recurring'),
            txt = '$'..comma_value(data.paymentAmount)
        },
        {
            header = Lang:t('menu.pay'),
            params = {
...
RegisterNetEvent('qb-vehicleshop:client:financePayment', function(data)
    local dialog = exports['qb-input']:ShowInput({
        header = Lang:t('menu.veh_payment'),
        submitText = Lang:t('menu.pay'),
        inputs = {
            {
                type = 'number',
                isRequired = true,
                name = 'paymentAmount',
                text = Lang:t('menu.pay_amount')
            }
        }
    })
    if dialog then
